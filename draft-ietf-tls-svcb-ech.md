...
The name of the destination domain is protected from disclosure by ECH, but it is still revealed by the DNS queries used to reach the server.  To achieve the security goals from {{Section 10.1 of !ECH}}, clients for this specification SHOULD select a DNS resolver that they trust to preserve the confidentiality of their queries and return authentic answers, and use authenticated encryption for DNS queries and responses.
