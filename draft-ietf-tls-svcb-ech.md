...
ECH ensures that TLS does not disclose the SNI, but the same information is also revealed in the DNS queries used to resolve the server's hostname.  To achieve the security goals from {{Section 10.1 of !ECH}}, clients for this specification SHOULD use an authenticated and encrypted connection to a DNS resolver that they trust to preserve the confidentiality of their queries and return authentic answers.
